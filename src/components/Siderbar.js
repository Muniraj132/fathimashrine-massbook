...
export default function Siderbar() {
  ...
  const paymentUrl = "https://payonline.fathimashrine.com/";

  function hadleSubmit(e) {
    const datas = {
      amount: 150 * filteredEvents.length,
    ...
    window.location =
      paymentUrl +
      `payment/?amount=${datas.amount}&name=${filteredEvents[0].name}&email=${filteredEvents[0].email}&mobile=${filteredEvents[0].number}&intention=${datas.intention}&intentionfor=${datas.intentionfor}&mass_id=${datas.mass_id}&massTiming=${datas.massTiming}&language=${datas.language}`;
    axios
  ...
  const getUrl = "https://massbooking.fathimashrine.com/?f_code=";
      ...
      .then((res) => {
        if (data[1] === "OK") {
          swal({
            ...
            text: "Our Lady of Fatima Shrine, Krishnagiri",
            icon: "success",
        ...
        <table>
          ...
          <div className="flex items-center justify-end p-2 border-t border-solid border-blueGray-200 rounded-b">
            <button
              type="button"
              onClick={hadleSubmit}
              class="text-white bg-blue-500 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
              <svg
            ...
            <p style={{ paddingLeft: "8%" }}>
              Total : ₹ {150 * filteredEvents.length}
            </p>
          </div>
        </table>
      )}
